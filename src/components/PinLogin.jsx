...
export default function PinLogin({ users, onLogin, error, busy }) {
  ...
  function pressKey(digit) {
    if (!selected || pin.length >= PIN_LENGTH || busy || verifying) return
    const next = pin + digit
    setPin(next)
    if (next.length === PIN_LENGTH) {
      setVerifying(true)
      onLogin(selected.id, next, () => {
        setVerifying(false)
    ...
    <div className="login-shell">
      ...
      {selected && (
        <>
          ...
          {error && !verifying && <div className="error-line">{error}</div>}
        </>
      )}
    </div>
  )
